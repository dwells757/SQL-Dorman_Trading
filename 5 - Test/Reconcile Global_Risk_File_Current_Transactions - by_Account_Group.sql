...
SET @Account_Group='SILVERCAPE_INVSMT'
...
B.PBS,
SUM(B.PQTY) --Quantity
FROM [dbo].[Account_Groups] A
INNER JOIN [dbo].[Global_Risk_File_Current_Transactions] B
ON (A.Account=B.Account)
WHERE (A.Account_Group=@Account_Group)
AND (B.IN_GMI_Contracts_SOD='Y')
GROUP BY
B.PSDATE,
B.Account,
B.Related_Account, --WARNING: This may cause a PRIMARY KEY violation!!!
B.[Product],
B.PBS
...
SUM(B.PQTY) --Quantity
FROM [dbo].[Account_Groups] A
INNER JOIN [dbo].[Global_Risk_File_Current_Transactions_2] B
ON (A.Account=B.Account)
WHERE (A.Account_Group=@Account_Group)
AND (B.IN_GMI_Contracts_SOD='Y')
GROUP BY
